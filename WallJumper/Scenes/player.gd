...
#	print(state)
	match state:
		...
		States.WALL:
			...
			# Add Wall Jump
			if Input.is_action_pressed("jump") and ((Input.is_action_pressed("left") and direction == Direction.RIGHT ) or ( Input.is_action_pressed("right") and direction == -1)):
...
func set_direction():
	direction = Direction.RIGHT if not $AnimatedSprite2D.flip_h else Direction.LEFT
